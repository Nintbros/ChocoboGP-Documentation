...
## `TrackingPoints` (wip)
Array containing a couple hundred 3D coordinate sets that each make up 3 lines through the track: one to tell the game where the middle is, where the right wall (or otherwise where main road ends and offroad begins) is, and where the left wall/offroad is. Every `TrackingPoint` must dictate which is the next `TrackingPoint` and which is the previous one, as well as the distance between them. It is unclear what effect the listed distance has on gameplay, if any. They also have a property named `UpVector` which dictates the current "gravity" for the current TrackingPoint. Yes, the funny chicken race has MK8 anti gravity support coded into it. Unfortunately rapid flips in gravity seem to "lock" the karts to a certain gravity value, so custom tracks can't get too crazy with it. Should work fine otherwise however. (MK8 Mute City port when?)
